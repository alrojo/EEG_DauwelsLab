...
tb_train = load_gz('./data/y_train.npy.gz')
xb_valid = load_gz('./data/X_val.npy.gz')
tb_valid = load_gz('./data/y_val.npy.gz')
xb_test = load_gz('./data/X_test.npy.gz')
tb_test = load_gz('./data/y_test.npy.gz')
xs_train = xb_train
ts_train = tb_train
xs_valid = xb_valid
ts_valid = tb_valid
...
for epoch in range(num_epochs):
    if 1==1:#(i + 1) % config.validate_every == 0:
        ...
        for subset, xb, xs, tb, ts, all_accuracy, all_auc in sets:
            ...
            acc_eval = T.mean(T.eq(T.argmax(predictions, axis=1), y),
    ...
    acc_train = T.mean(T.eq(T.argmax(predictions, axis=1), labels),
                      dtype=theano.config.floatX)
